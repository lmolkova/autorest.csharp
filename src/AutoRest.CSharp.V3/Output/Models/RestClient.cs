...
namespace AutoRest.CSharp.V3.Output.Models
{
    internal class RestClient : ClientBase
    {
        ...
        private RestClientMethod BuildMethod(Operation operation, HttpRequest httpRequest, ICollection<RequestParameter> requestParameters, ResponseHeaderGroupType? responseHeaderModel)
        {
            ...
            List<(RequestParameter, ReferenceOrConstant)>? bodyParameter = new List<(RequestParameter, ReferenceOrConstant)>();
                ...
                if (requestParameter.Protocol.Http is HttpParameter httpParameter)
                {
                    ...
                    switch (httpParameter.In)
                    {
                        ...
                        case ParameterLocation.Body:
                            bodyParameter.Add((requestParameter, constantOrReference));
                            break;
            ...
            if (bodyParameter.Count > 0 && bodyParameter[0] is var (bodyRequestParameter, bodyParameterValue))
            {
                ...
                else if (httpRequestWithBody.KnownMediaType == KnownMediaType.Multipart)
                {
                    ...
                    foreach (var parameter in bodyParameter)
                    {
                        (bodyRequestParameter, bodyParameterValue) = parameter;
                        if (bodyParameterValue.Type.Name == "String")
                        {
