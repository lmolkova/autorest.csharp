...
namespace AnomalyDetector.Models
{
    ...
    internal partial class MongoDBDataFeed : DataFeedDetail
    {
        ...
        internal MongoDBDataFeed(string dataFeedName, Granularity granularityName, IEnumerable<Metric> metrics, IEnumerable<Dimension> dimension, DateTimeOffset dataStartFrom, MongoDBParameter dataSourceParameter) : base(dataFeedName, granularityName, metrics, dimension, dataStartFrom)
        {
            ...
            DataSourceType = new DataSourceType("MongoDBDataFeed");
        }
        ...
        /// <param name="dataSourceParameter"> . </param>
        internal MongoDBDataFeed(DataSourceType dataSourceType, Guid? dataFeedId, string dataFeedName, string dataFeedDescription, Granularity granularityName, int? granularityAmount, IReadOnlyList<Metric> metrics, IReadOnlyList<Dimension> dimension, DateTimeOffset dataStartFrom, string timestampColumn, long? startOffsetInSeconds, int? maxConcurrency, long? minRetryIntervalInSeconds, long? stopRetryAfterInSeconds, NeedRollupEnum? needRollup, RollUpMethod? rollUpMethod, string rollUpColumns, string allUpIdentification, FillMissingPointType? fillMissingPointType, double? fillMissingPointValue, ViewMode? viewMode, IReadOnlyList<string> admins, IReadOnlyList<string> viewers, bool? isAdmin, string creator, EntityStatus? status, DateTimeOffset? createdTime, string actionLinkTemplate, MongoDBParameter dataSourceParameter) : base(dataSourceType, dataFeedId, dataFeedName, dataFeedDescription, granularityName, granularityAmount, metrics, dimension, dataStartFrom, timestampColumn, startOffsetInSeconds, maxConcurrency, minRetryIntervalInSeconds, stopRetryAfterInSeconds, needRollup, rollUpMethod, rollUpColumns, allUpIdentification, fillMissingPointType, fillMissingPointValue, viewMode, admins, viewers, isAdmin, creator, status, createdTime, actionLinkTemplate)
        {
            DataSourceParameter = dataSourceParameter;
            DataSourceType = dataSourceType;
        }

        public MongoDBParameter DataSourceParameter { get; }
    }
}
